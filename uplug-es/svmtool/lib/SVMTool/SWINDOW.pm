...
sub clear
{
    #description _ clears the whole window
    #param1 _ window reference  (implicit)
...
sub get_last_word
{
    #description _ returns the LAST WORD in the given window
    #param1 _ window reference  (implicit)

    my $rwin = shift;

    return $rwin->get_word($rwin->{len}-1);
}
...
sub prepare
{
    ...
    if ($direction eq $COMMON::lrmode) {
       # look for EOS position (if it exists) to the right
       my $i = $rwin->get_core;
       my $eos = 0;
       while (($i < $rwin->get_len) and (!$eos)) {
          if (COMMON::end_of_sentence($rwin->get_word($i))) { $eos = 1; }
          $i++;
       }
       # delete out of sentence items
       while ($i < $rwin->get_len) {
          $rwin->reset($i);
          $i++;
       }
       # look for EOS position (if it exists) to the left
       $i = $rwin->get_core - 1;
       $eos = 0;
       while (($i >= 0) and (!$eos)) {
          if (COMMON::end_of_sentence($rwin->get_word($i))) { $eos = 1; }
    ...
    else { die "[ERROR] unknown direction '$direction'\n"; }
...
sub print_stdout
{
    ...
    while ($i < $rwin->get_len) {
       print STDOUT "W", $i - $rwin->get_core, ": \t";
       printf STDOUT "%-20s %-20s %-5s %-20s", $rwin->{win}[$i]->[1], "(".$rwin->{win}[$i]->[4].")", $rwin->{win}[$i]->[2], (($rwin->{win}[$i]->[3]) == $COMMON::emptypos ? $rwin->{win}[$i]->[3] : join($COMMON::innerseparator, @{$rwin->{win}[$i]->[3]}));
       my $j = 5;
       while ($j < scalar(@{$rwin->{win}[$i]})) {
          printf STDOUT " %-10s", $rwin->{win}[$i]->[$j];
          $j++;
       }
       printf STDOUT "\n";
       $i++;
