...
sub get_out
{
     ...
     print STDERR "  - D     <distance script options> -> options for the distance module (without saving the new dictionary) \n";
     print STDERR "  - DS     <distance script options> -> options for the distance module (saving the new dictionary) \n";
...
while (($i < $ARGLEN) and (!$ARGOK)) {
   ...
   elsif (($opt eq "-D") or ($opt eq "-d")) { $distance = shift(@ARGV); }
   elsif (($opt eq "-DS") or ($opt eq "-ds")) { $distance = shift(@ARGV); $save_dicc=1}
   else {
...
# ================================================================================================
  ...
  # first of all...see if we have to apply the distance module....
  if ($distance) {
     print STDERR "----------------------------------------------------------------------------------------\n$COMMON::appname v$COMMON::appversion\n(C) $COMMON::appyear TALP RESEARCH CENTER.\nWritten by Jesus Gimenez and Lluis Marquez.\n----------------------------------------------------------------------------------------\n";
     print STDERRR "----------------------------------------------------------------------------------------\n ";
     print STDERR "Applying the distance module\n";

      #we call the distance script and apply the changes to the model 
      #we save the current dictionary of the model
      my $copy_order="cp " . $corpus . ".DICT " . $corpus . ".DICT_old";
      system($copy_order);
      #we create the new dictionary

      my $distance_statement="python \$HOME/svmtool/scripts/dist_edit_dicc.py ".$distance ." >> " . $corpus . ".DICT";# . $distance . " " .$corpus . ".DICT " .$corpus . ".DICT";
      system($distance_statement);      
     print STDERR "[DONE]\n";
     print STDERR "----------------------------------------------------------------------------------------\n";
  }
  ...
  #if distance, we left the dictionary of the model as the user had it
  if ($save_dicc==1){
      my $save_order="cp " . $corpus . ".DICT " . $corpus . ".DICT_file";
      my $copy_order2="cp " . $corpus . ".DICT_old " . $corpus . ".DICT";
      system($save_order);
      system($copy_order2);
  }else{
      my $copy_order2="cp " . $corpus . ".DICT_old " . $corpus . ".DICT";
