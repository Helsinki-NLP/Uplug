...
static connect_rule_t **connect_mtr1;  /* original connection matrix */
static connect_rule_t **connect_mtr2;  /* compressed connection matrix */
...
get_pair2(chasen_cell_t * cell, rensetu_pair2_t * pair)
{
    ...
    if (strmatch2(s, JSTR_BOS, ESTR_BOS) ||
	strmatch2(s, JSTR_EOS, ESTR_EOS)) {
	pair->hinsi = NULL;
	return;
    }
    pair->hinsi = cell_p;
...
write_matrix(void)
{
    ...
    for (i = 0; i < i_num; i++) {
	int nval = 0;
	int next0 = connect_mtr1[i][0].next;
	int cost0 = connect_mtr1[i][0].cost;
	for (j = 0; j < j_num; j++) {
	    if (connect_mtr1[i][j].next == next0 &&
		connect_mtr1[i][j].cost == cost0) {
		nval++;
	    } else {
		if (next0 == 0 && cost0 == 0)
		    fprintf(fp, "o%d ", nval);
		...
		nval = 1;
		next0 = connect_mtr1[i][j].next;
		cost0 = connect_mtr1[i][j].cost;
	    }
	}
	if (nval > 0) {
	    if (next0 == 0 && cost0 == 0)
...
in rensetu_pair_t       v
  ...
  * matrix.cha
  compressed connection matrix -- `connect_mtr2' in mkchadic/makemat.c
  connect_mtr1 is compressed to connect_mtr2.
