...
language-specific tools and models (C<xx> = language ID)

=item C<uplug-treetagger>: 

integration of the TreeTagger

=item C<uplug-webalign>: 

webinterface for interactive alignment (ICA, ISA)

=item C<uplug-cwb>: 

tools for indexing parallel corpora for the CWB
...
You may also download the latest source from bitbucket and install all packages in one run:
 ...
 git clone https://bitbucket.org/tiedeman/uplug.git
 cd uplug
 make all
 sudo make install
