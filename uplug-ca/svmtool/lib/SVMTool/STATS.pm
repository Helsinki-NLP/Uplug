...
sub do_statistics
{
    ...
    while (defined(my $in = $INPUT->getline())) {
       $in = COMMON::trim($in);
       while (defined($in) and ($in =~ /^$COMMON::SMARK$/)) { # skip EOS markers
       	  $in = $INPUT->getline();
          $in = COMMON::trim($in);
       }
       if (defined(my $out = $OUTPUT->getline())) {
          $out = COMMON::trim($out);
          while (defined($out) and ($out =~ /^$COMMON::SMARK$/)) { # skip EOS markers
             $out = $OUTPUT->getline();
             $out = COMMON::trim($out);
          }
          chomp($in);
          ...
          if ((($linein[0] ne $COMMON::IGNORE) and ($linein[0] ne "")) and
              (($lineout[0] ne $COMMON::IGNORE) and ($lineout[0] ne "")) and
              ($linein[0] ne $COMMON::SMARK) and
              (scalar(@linein) > 1)) {
             $nsamples++;
             if ($verbose > $COMMON::verbose2) { COMMON::show_progress($nsamples, $COMMON::progress1, $COMMON::progress2); }

#             if ($wordin eq "?") { # special treatment of "?" question mark (always correct)
#	             $nhits++; $hit = 1;
#             }
#             else {
